import { initFromDevice, surface, type Gpu, type Surface } from "vgpu";
...
export function createRenderer({ canvas, initialControls }: RendererOptions) {
  ...
  const initialize = async () => {
    if (disposed) return;
    const sharedDevice = await getSharedGpuDevice();
    const nextGpu = await initFromDevice(sharedDevice);
    if (disposed) {
      nextGpu.dispose();
      return;
    }
