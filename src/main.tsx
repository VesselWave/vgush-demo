...
import { RadianceCanvas } from './radiance/RadianceCanvas'
import './styles.css'

const demos: { scene: Scene | 'radiance'; name: string; description: string; detail: string }[] = [
  ...
  { scene: 'radiance', name: 'Radiance Cascades', description: 'Draw colored light into a field and watch it spread across the grid around a fixed white-hot monolith.', detail: 'Persistent light field · Multi-pass propagation · Based on the vgpu example' },
]
      ...
      <section className="gallery" id="experiments" aria-label="WebGPU experiments">
        {demos.map((demo, index) => <article className={`demo demo-${index + 1}`} key={`${demo.scene}-${key}`}>
          {demo.scene === 'prism'
            ...
            : demo.scene === 'radiance'
              ? <RadianceCanvas intensity={running ? 1 : .25} />
